...
public:
	...
	explicit Vector2(const float inX, const float inY) : x(inX), y(inY)
	{
	}
...
public:
	...
	explicit Vector3(const float inX, const float inY, const float inZ) : x(inX), y(inY), z(inZ)
	{
	}
...
public:
	...
	{
		const float yScale = Math::Cot(fovY / 2.0f);
...
public:
	...
	{
		const float scalar = Math::Sin(angle / 2.0f);
		x = axis.x * scalar;
		y = axis.y * scalar;
		z = axis.z * scalar;
